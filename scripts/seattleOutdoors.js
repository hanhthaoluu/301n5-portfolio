'use strict';

var rawData = [
  {
    place: '<em>Mount Rainier</em>',
    activities: '<p>Hike Mount Rainier - Paradise in July to see the colorful field of wild flowers. See if you can spot deers and foxes while you are there.</p>',
    address: '55210 238th Avenue East, Ashford, WA 98304',
    website: 'https://www.nps.gov/mora/planyourvisit/paradise.htm'
  },
  {
    place: '<em>Alki Beach</em>',
    activities: '<p>Stroll along the Alki Beach for the glorious sunset.  The view of the downtown Seattle skyline is unbeatable from Alki. During the summer months, take the water taxi from the Alki Beach to downtown Seattle. At Alki, if you look close enough, you can spot some seals, sea otters and even owls.</p>',
    address: '1702 Alki Ave SW, Seattle, WA 98116',
    website: 'https://www.seattle.gov/parks/find/parks/alki-beach-park'
  },
  {
    place: '<em>Carkeek Park</em>',
    activities: '<p>View the breathtaking view of the Puget Sound up high from the Carkeek Park Play Area and then walk down to the beach. During the lowest tides you can admire starfish, sea cucumbers, sea anemones, crabs, and other sea creatures in their natural habitats.</p>',
    address: '950 NW Carkeek Park Rd, Seattle, WA 98177',
    website: 'https://www.seattle.gov/parks/find/parks/carkeek-park'
  },
  {
    place: '<em>Washington Park Arboretum</em>',
    activities: '<p>Take a walk at the Washington Park Arboretum in May along the glowing Azeleas and Rhododendron trails.</p>',
    address: '2300 Arboretum Dr E, Seattle, WA 98112',
    website: 'https://botanicgardens.uw.edu/washington-park-arboretum/'
  },
  {
    place: '<em>Olympic Farm Game</em>',
    activities: '<p>Go to the Olympic Farm Game to visit all the exotic animals and feed the goats.</p>',
    address: '1423 Ward Rd, Sequim, WA 98382',
    website: 'http://olygamefarm.com/'
  },
  {
    place: '<em>Lake Crescent</em>',
    activities: '<p>When it\'s 90s degrees in Seattle, go to the Lake Crescent, up north, near the Canadian border.  Don\'t forget to stop by the Sol Duc Hotspring while you are up there. Lake Crescent is the most beautiful and crystal clear lake in the NW.</p>',
    address: '600 E. Park Avenue, Port Angeles, WA 98362',
    website: 'https://www.nps.gov/olym/planyourvisit/visiting-lake-crescent.htm'
  },
  {
    place: '<em>Crystal Mountain</em>',
    activities: '<p>In the summer, take the gondola up to the top of Crystal Mountain between late June to mid September. Looking out from the gondola on your way up to the Crystal Mountain is the most transquil and unforgetable experience ever.</p>',
    address: '33914 Crystal Mountain Blvd, Enumclaw, WA 98022',
    website: 'https://crystalmountainresort.com/summer-activities/scenic-gondola-rides/'
  },
  {
    place: '<em>UW Quad Cherry Blossoms</em>',
    activities: '<p>In April, see the cherry blossoms at the UW Quad. Remember to track the blooms with Cherry Blossoms Live Stream at http://uwtv.org/uw-quad-cherry-blossom-cam/ so you don\'t miss the special moment. April showers and winds tend to blow the blossoms away very quickly.</p>',
    address: '4060 George Washington Lane Northeast, Seattle, WA 98195',
    website: 'http://www.washington.edu/externalaffairs/2017/03/22/cherry-blossoms-live-stream/'
  },
  {
    place: '<em>Tulips Festival</em>',
    activities: '<p>During the month of April, Tulips Festival is a must. Pictures don\'t do it justice! Remember to take an earlier exit, Exit 225 (I believe) instead of Exit 226 and take the back roads from there, so you are not stuck in a line of cars for one hour trying to get there.</p>',
    address: '311 W Kincaid St, Mt Vernon, WA 98273',
    website: 'www.tulipfestival.org'
  },
  {
    place: '<em>Flower World</em>',
    activities: '<p>Visit Flower World, a fifteen acres nursery/animal farm/farm market, to find exotic plants and feed the chickens, sheeps and goats. Bring your camera for the peacock dances.</p>',
    address: '9322 196th St SE, Snohomish, WA 98296',
    website: 'www.flowerworldusa.com'
  },
];
